  ...
  <page-header-wrapper>
    <a-card :bordered="false">
      ...
      <div class="table-operator">
        <a-button type="primary" icon="plus" @click="handleAdd">新建 {{ $auth('dashboard.add') }}</a-button>
          ...
          <a-list :grid="{ gutter: 16, xs: 1, sm: 2, md: 4, lg: 4, xl: 6, xxl: 3 }" :data-source="record.operators">
            <a-list-item slot="renderItem" slot-scope="item, index">
              <a-card :title="item.menuName">
                <a-checkbox-group  v-model="item.alreadyActions">
                  <a-row>
                    <a-col :span="8" v-for="btn in JSON.parse(item.actions)" :key="btn">
                      <a-checkbox :value="btn" @change="(data) => {
...
export default {
  ...
  methods: {
    ...
    onChange(data, { menuId,roleId }) {


      roleModifyOperatorMenu({
        operatorId: data,menuId,roleId
